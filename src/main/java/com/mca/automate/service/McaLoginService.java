...
public class McaLoginService {
    ...
    public LoginResponse login(String username, String password, String deviceId, String requestType, String captcha, String preCt, String cookie) {
        synchronized (this.loginLockFor(username)) {
            return this.doLogin(username, password, deviceId, requestType, captcha, preCt, cookie);
        }
    }

    private LoginResponse doLogin(String username, String password, String deviceId, String requestType, String captcha, String preCt, String cookie) {
        try {
            String loginCookie;
            String captchaVal;
            String preCtVal;

            if (captcha != null && !captcha.isBlank() && preCt != null && !preCt.isBlank() && cookie != null && !cookie.isBlank()) {
                loginCookie = this.withDeviceCookie(cookie, deviceId);
                captchaVal = captcha;
                preCtVal = preCt;
            } else {
                ...
                loginCookie = cap.cookie();
                captchaVal = cap.captcha();
                preCtVal = cap.preCt();
            }

            String loginPayLoad = "userName=" + username.toUpperCase() + "&password=" + this.customSHA.calcSHA(password) + "&deviceId=" + deviceId + "&userInput=" + this.util.stripNewlines(this.util.safe(captchaVal)) + "&pre_CT=" + this.util.stripNewlines(this.util.safe(preCtVal)) + "&paramData=null";
            ...
            try (Response loginResponse = loginRequestExecuter(LOGIN_URL, encryptedData, loginCookie)) {
                ...
                if ("208".equalsIgnoreCase(resCode)) {
                    ...
                    String forceLoginPayLoad = "previousSession=killActiveSession&userName=" + username.toUpperCase() + "&password=" + this.customSHA.calcSHA(password) + "&deviceId=" + deviceId + "&userInput=" + this.util.stripNewlines(this.util.safe(captchaVal)) + "&pre_CT=" + this.util.stripNewlines(this.util.safe(preCtVal)) + "&paramData=null";
                ...
                if ("206".equalsIgnoreCase(resCode)) {
                    ...
                    this.otpSessionStore.remember(username, newCookie3, mobile, sblUserId);
    ...
    public String verifiedOTPLogin(VerifyOtpDTO verifyOtpDTO) {
        ...
        try (Response response = loginRequestExecuter(LOGIN_URL, encryptedData, verifyOtpDTO.getCookie())) {
            ...
            if (response.code() == 200) {
                ...
                finalCookie = this.withDeviceCookie(finalCookie, verifyOtpDTO.getDeviceId());
